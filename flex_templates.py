...
# --------------------------------------------------
# ألوان احترافية ومريحة للعين مع تدرجات وظلال
# --------------------------------------------------
COLORS = {
    "primary": "#5E35B1",       # أرجواني غامق
    "secondary": "#7E57C2",     # أرجواني فاتح
    "accent": "#FF4081",         # وردي نابض
    "bg": "#F5F5F5",             # خلفية فاتحة
    "text": "#212121",           # نص أسود ناعم
    "muted": "#757575",          # نص ثانوي
    "white": "#FFFFFF",
    "shadow": "#D1C4E9"          # ظل لطيف للثري دي
}
...
def create_header(text_value):
    return {
        ...
        "layout": "vertical",
        "contents": [
            {"type": "text", "text": text_value, "size": "lg", "weight": "bold", "align": "center", "color": COLORS["white"]}
        ],
        "paddingAll": "15px",
...
def create_card_display(card, is_daily=False, is_learning=False):
    ...
    return {
        ...
        "body": {
            ...
            "layout": "vertical",
            "contents": [
                {"type": "text", "text": "✨ المعنى", "size": "sm", "weight": "bold", "color": COLORS["primary"], "margin": "none"},
    ...
    bubbles.append({
        ...
        "body": {
            ...
            "contents": [
                ...
                create_separator(),
                {"type": "text", "text": f"📅 {format_time(result.get('timestamp', ''))}", "size": "xs", "color": COLORS["muted"], "align": "center"}
            ],
            "paddingAll": "20px",
            "cornerRadius": "md",
            "shadow": "sm"
...
def create_celtic_cross_result(result):
    positions = ["1️⃣ الوضع الحالي", "2️⃣ التحدي", "3️⃣ السبب الجذري", "4️⃣ الماضي",
    ...
    for i, card in enumerate(result["cards"][:10]):
        ...
        if len(meaning) > 180: meaning = meaning[:177] + "..."
        bubbles.append({
            "type": "bubble",
            ...
            "body": {"type": "box", "layout": "vertical", "contents": [{"type": "text", "text": meaning, "size": "sm", "color": COLORS["text"], "wrap": True}], "paddingAll": "15px", "cornerRadius": "md", "shadow": "sm"}
        })
    bubbles.append({
        "type": "bubble",
        "header": create_header("✨ الملخص النهائي"),
        "body": {"type": "box", "layout": "vertical", "contents": [{"type": "text", "text": result.get("interpretation", "تحليل شامل للقراءة"), "size": "sm", "color": COLORS["text"], "wrap": True}], "paddingAll": "20px"},
        "footer": {"type": "box", "layout": "vertical", "contents": [create_button("↩️ القائمة الرئيسية", "action=main_menu", style="link")], "paddingAll": "10px"}
    })
    return {"type": "carousel", "contents": bubbles}
    ...
    return {
        ...
        "body": {
            ...
            "contents": [
                ...
                {"type": "text", "text": "قريباً: معرض البطاقات الكامل", "size": "xs", "color": COLORS["muted"], "align": "center", "margin": "md"}
            ],
            "paddingAll": "20px",
            "backgroundColor": COLORS["bg"]
        },
...
def create_stats_view(stats):
    ...
    return {
        ...
        "hero": {
            ...
            "layout": "vertical",
            "contents": [
                {"type": "text", "text": level, "size": "xl", "weight": "bold", "align": "center", "color": COLORS["white"]},
                {"type": "text", "text": f"إجمالي النشاط: {total}", "size": "sm", "align": "center", "color": COLORS["accent"], "margin": "md"}
            ],
            "paddingAll": "20px",
            "backgroundColor": COLORS["primary"],
            "cornerRadius": "lg",
